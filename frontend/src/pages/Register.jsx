...
import { UserPlus, Mail, Lock, User, Loader2, AlertCircle, Sparkles, ShieldCheck, Command } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 sm:px-6 pt-32 pb-20 bg-black">
      <div className="w-full max-w-lg relative">
        ...
        >
          <div className="absolute top-0 left-0 w-32 h-32 bg-white/[0.05] blur-3xl rounded-full" />
          ...
          <motion.div variants={itemVariants} className="text-center mb-10 sm:mb-12">
            <div className="relative inline-block group mb-6 sm:mb-8">
              ...
              <div className="relative w-16 h-16 sm:w-20 sm:h-20 bg-white rounded-2xl sm:rounded-[2rem] flex items-center justify-center mx-auto shadow-2xl border border-white/10">
                <Command className="w-8 h-8 sm:w-10 sm:h-10 text-black" />
              </div>
            </div>
            <h1 className="text-3xl sm:text-4xl font-display font-black text-white mb-3 tracking-tight italic uppercase">Join Vault<span className="text-white/20 not-italic">.</span></h1>
            <p className="text-white/20 text-xs sm:text-sm font-black uppercase tracking-[0.3em] italic">New Operative Registration</p>
          </motion.div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
            <motion.div variants={itemVariants} className="space-y-3">
              <label className="text-[10px] font-black text-white/20 ml-4 tracking-[0.4em] uppercase italic">Designation</label>
              <div className="relative group">
                <User className="absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 text-white/20 group-focus-within:text-white transition-colors" />
                <input 
                  type="text"
                  required
                  value={name}
            ...
            <motion.div variants={itemVariants} className="space-y-3">
              <label className="text-[10px] font-black text-white/20 ml-4 tracking-[0.4em] uppercase italic">Access Key</label>
              <div className="relative group">
              ...
              <div className="relative flex items-center justify-center space-x-3 text-black font-black text-base sm:text-lg uppercase tracking-[0.2em]">
                {loading ? <Loader2 className="w-6 h-6 animate-spin" /> : (
                  <>
                    ...
                    <ShieldCheck className="w-5 h-5 transition-transform duration-500 group-hover:rotate-12" />
                  </>
                )}
              </div>
            </motion.button>
          ...
          <motion.div variants={itemVariants} className="mt-10 sm:mt-12 text-center">
            <p className="text-white/20 font-black text-[10px] sm:text-xs uppercase tracking-[0.3em] italic">
              Already verified?{' '}
              <Link to="/login" className="text-white hover:text-white/60 font-black transition-colors ml-2 border-b border-white/20 pb-0.5">
                Access Vault
              </Link>
            </p>
          </motion.div>
